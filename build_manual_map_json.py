...
def set_edge(nodes, src, dst, **attrs):
    ...
    naval_facility_ids = target.get("naval_facility_ids") or []
    if isinstance(naval_facility_ids, str):
        naval_facility_ids = [naval_facility_ids]
    else:
        naval_facility_ids = list(naval_facility_ids)

    if naval_facilities and not naval_facility_ids:
        land_node = src if src.startswith("land_") else (dst if dst.startswith("land_") else src)
        land_part = zone_name_part(land_node)
        naval_facility_ids = [f"facility_{land_part}_{facility}" for facility in naval_facilities]

    if len(naval_facility_ids) != len(naval_facilities):
        raise ValueError(
            f"Edge {src}->{dst} naval_facility_ids length ({len(naval_facility_ids)}) "
            f"must match naval_facilities length ({len(naval_facilities)})."
        )
...
def link_land_sea(
    ...
    naval_facility_id=None,
    naval_facility_ids=None,
    railway_connection=False,
    rail_id=None,
    passable=True,
    canal=None,
    ...
    if naval_facility_ids is None:
        if naval_facility_id is not None:
            if len(naval_facilities) != 1:
                raise ValueError("naval_facility_id requires exactly one naval facility type.")
            naval_facility_ids = [naval_facility_id]
        else:
            land_part = zone_name_part(land)
            naval_facility_ids = [f"facility_{land_part}_{facility}" for facility in naval_facilities]
    elif isinstance(naval_facility_ids, str):
        naval_facility_ids = [naval_facility_ids]
    else:
        naval_facility_ids = list(naval_facility_ids)
    ...
    if len(naval_facility_ids) != len(naval_facilities):
        raise ValueError(
            f"link_land_sea {land}<->{sea}: naval_facility_ids length ({len(naval_facility_ids)}) "
            f"must match naval_facilities length ({len(naval_facilities)})."
        )
...
def build():
    ...
    land_nodes = [
        ...
        ("land_india_punjab", "Punjab", "normal", 0, "FEC", False, False),
        ("land_india_delhi", "Delhi", "city", 1, "FEC", False, False),
        ("land_india_maharashtra", "Maharashtra", "normal", 1, "FEC", False, False),
        ("land_india_benares", "Benares", "normal", 1, "FEC", False, False),
        ("land_india_calcutta", "Calcutta", "city", 3, "FEC", False, False),
        ("land_india_southern_india", "Southern India", "normal", 1, "FEC", False, False),
        ("land_india_ceylon", "Ceylon", "normal", 0, "FEC", False, False),
        ("land_burma", "Burma", "jungle", 1, "FEC", False, False),
        ...
        ("land_can_district_of_keewatin", "District of Keewatin", "tundra", 0, "United Kingdom", False, False),
        ("land_can_baffin_island", "Baffin Island", "tundra", 0, "United Kingdom", False, False),
        ("land_can_manitoba", "Manitoba", "normal", 1, "United Kingdom", False, False),
        ("land_can_ontario", "Ontario", "normal", 2, "United Kingdom", False, False),
        ("land_can_quebec", "Quebec", "normal", 1, "United Kingdom", False, False),
        ("land_can_labrador", "Labrador", "normal", 1, "United Kingdom", False, False),
        ("land_can_newfoundland", "Newfoundland", "normal", 1, "United Kingdom", False, False),
        ("land_can_nova_scotia", "Nova Scotia", "normal", 1, "United Kingdom", False, False),
        ("land_can_st_pierre_island", "St. Pierre Island", "normal", 0, "France", False, False),
        ("land_can_british_columbia", "British Columbia", "mountain", 1, "United Kingdom", False, False),
        ("land_can_alberta_saskatchewan", "Alberta-Saskatchewan", "normal", 0, "United Kingdom", False, False),
        ...
        ("land_new_guinea", "New Guinea", "jungle", 1, "ANZAC", False, False),
        ("land_new_britain", "New Britain", "jungle", 1, "ANZAC", False, False),

        ("land_aus_western_australia", "Western Australia", "desert", 0, "ANZAC", False, False),
        ("land_aus_northern_territory", "Northern Territory", "desert", 0, "ANZAC", False, False),
        ("land_aus_south_australia", "South Australia", "desert", 0, "ANZAC", False, False),
        ("land_aus_queensland", "Queensland", "normal", 1, "ANZAC", False, False),
        ("land_aus_new_south_wales", "New South Wales", "normal", 2, "ANZAC", False, False),
        ("land_aus_tasmania", "Tasmania", "normal", 0, "ANZAC", False, False),
        ("land_nz_north_island", "North Island", "normal", 1, "ANZAC", False, False),
        ("land_nz_south_island", "South Island", "normal", 0, "ANZAC", False, False),
    ...
    link(nodes, "sea_a7", "sea_a10", border_terrain="sea")
    link(nodes, "sea_a10", "sea_a11", border_terrain="sea")
    link(nodes, "sea_a11", "sea_a12", border_terrain="sea")
    link(nodes, "sea_a12", "sea_a13", border_terrain="sea")
    link(nodes, "sea_a13", "sea_a14", border_terrain="sea")
    link(nodes, "sea_a20", "sea_a21", border_terrain="sea")
    link(nodes, "sea_a21", "sea_a23", border_terrain="sea")
    link(nodes, "sea_a22", "sea_a23", border_terrain="sea")
    link(nodes, "sea_a22", "sea_a28", border_terrain="sea")
    link(nodes, "sea_a28", "sea_a29", border_terrain="sea")

    link(nodes, "sea_m1", "sea_a44", border_terrain="sea", canal_or_strait="Strait of Gibraltar")
    link(nodes, "sea_m1", "sea_m2", border_terrain="sea")
    link(nodes, "sea_m2", "sea_m3", border_terrain="sea")
    link(nodes, "sea_m2", "sea_m4", border_terrain="sea")
    link(nodes, "sea_m3", "sea_m5", border_terrain="sea")
    link(nodes, "sea_m4", "sea_m5", border_terrain="sea")
    link(nodes, "sea_m5", "sea_m10", border_terrain="sea")
    link(nodes, "sea_m10", "sea_m11", border_terrain="sea")
    link(nodes, "sea_m11", "sea_m12", border_terrain="sea")
    link(nodes, "sea_m10", "sea_m9", border_terrain="sea")
    link(nodes, "sea_m9", "sea_i3", border_terrain="sea")
    link(nodes, "sea_i3", "sea_i5", border_terrain="sea")
    link(nodes, "sea_i5", "sea_i4", border_terrain="sea")
    link(nodes, "sea_i3", "sea_i6", border_terrain="sea")
    ...
    link(nodes, "sea_a18", "sea_a19", border_terrain="sea")
    link(nodes, "sea_a18", "sea_a25", border_terrain="sea")
    link(nodes, "sea_a25", "sea_a26", border_terrain="sea")
    link(nodes, "sea_a26", "sea_a31", border_terrain="sea")
    link(nodes, "sea_a31", "sea_a33", border_terrain="sea")
    link(nodes, "sea_a33", "sea_a35", border_terrain="sea")
    link(nodes, "sea_a35", "sea_a38", border_terrain="sea")
    link(nodes, "sea_a38", "sea_a39", border_terrain="sea")
    link(nodes, "sea_a39", "sea_a40", border_terrain="sea")
    link(nodes, "sea_a36", "sea_a31", border_terrain="sea")
    link(nodes, "sea_a36", "sea_a33", border_terrain="sea")

    link(nodes, "sea_p5", "sea_p6", border_terrain="sea")
    link(nodes, "sea_p6", "sea_p11", border_terrain="sea")
    link(nodes, "sea_p11", "sea_p12", border_terrain="sea")
    link(nodes, "sea_p6", "sea_p12", border_terrain="sea")
    link(nodes, "sea_p12", "sea_p13", border_terrain="sea")
    link(nodes, "sea_p12", "sea_p24", border_terrain="sea")
    link(nodes, "sea_p13", "sea_p25", border_terrain="sea")
    link(nodes, "sea_p24", "sea_p25", border_terrain="sea")
    link(nodes, "sea_p25", "sea_p28", border_terrain="sea")
    link(nodes, "sea_p28", "sea_p30", border_terrain="sea")
    ...
    link(nodes, "sea_p60", "sea_p61", border_terrain="sea")
    link(nodes, "sea_p61", "sea_p62", border_terrain="sea")
    link(nodes, "sea_p62", "sea_p63", border_terrain="sea")
    link(nodes, "sea_p63", "sea_p64", border_terrain="sea")
    link(nodes, "sea_p64", "sea_p65", border_terrain="sea")
    link(nodes, "sea_p65", "sea_p68", border_terrain="sea")
    ...
    link_rail(nodes, "land_tibet", "land_nepal", border_terrain="mountain", passable=False)
    link_land(nodes, "land_tibet", "land_india_benares", border_terrain="mountain", passable=False)
    link_land(nodes, "land_tibet", "land_india_calcutta", border_terrain="mountain", passable=False)
    ...
    link(nodes, "land_can_baffin_island", "land_can_labrador", border_terrain="coast", narrow_crossing=True)
    ...
    link(nodes, "sea_a36", "sea_p31", border_terrain="sea", canal_or_strait="Panama Canal")
    ...
    link(nodes, "sea_m9", "sea_i5", border_terrain="sea", canal_or_strait="Suez Canal")
    ...
    link_land_sea(
        ...
        "land_china_shantung",
        "sea_p14",
        naval_facilities=["major_port", "minor_dockyard"],
    )
    link_land_sea(
        nodes,
        "land_china_nanking",
    ...
    link(nodes, "sea_a23", "sea_a12", border_terrain="sea", canal_or_strait="Kaiser-Wilhelm Kanal")
    ...
    for nid, node in nodes.items():
        ...
        for edge in node["neighbors"]:
            ...
            facility_ids = edge.get("naval_facility_ids") or []
            if isinstance(facility_ids, str):
                facility_ids = [facility_ids]
            else:
                facility_ids = list(facility_ids)

            if facilities and not facility_ids:
                land_node = nid if nid.startswith("land_") else (edge["neighbor_id"] if edge["neighbor_id"].startswith("land_") else nid)
                land_part = zone_name_part(land_node)
                facility_ids = [f"facility_{land_part}_{facility}" for facility in facilities]

            if len(facility_ids) != len(facilities):
                raise ValueError(
                    f"Edge {nid}->{edge['neighbor_id']} naval_facility_ids length ({len(facility_ids)}) "
                    f"must match naval_facilities length ({len(facilities)})."
                )

            edge["naval_facilities"] = facilities
            edge["naval_facility_ids"] = facility_ids
